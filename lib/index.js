...
const htmlTemplate = '<!DOCTYPE html><html lang="en-US"><head><meta charset="UTF-8" /><title>Containerized Status in NodeJS</title></head><body><h1>I am alive</h1><h3>%{date}</h3></body></html>';

function onReq(request, response) {
    out.i('recieved request');
    slUtilz.writeResponse(response, 200,  buildHtml(htmlTemplate, {date: datez.date()}));
}

function buildHtml(template = '<html></html>', vars = {}) {
    let result = template;
    out.i('using template: ' + result);
    Object.keys(vars).forEach(k => {
        out.i('examining key: ' + k);
        result = result.replace(new RegExp('%\{' + k + '\}', 'g'), (match) => {
            out.i('found result: '+vars[k]);
            return vars[k];
        });
    });

    out.i('final result: ' + result);
