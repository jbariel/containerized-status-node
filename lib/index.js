...
const svrPort = process.env.PORT || 8888;
const version = process.env.APP_VERSION || "unknown";

out.t("Using port " + svrPort);

const htmlTemplate =
  '<!DOCTYPE html><html lang="en-US"><head><meta charset="UTF-8" /><title>%{title}</title></head><body><h1>I am alive</h1><h3>%{date}</h3><h3>%{version}</h3></body></html>';

const defaultHtmlVals = {
  title: "Containerized Status in NodeJS",
  version: version
};

function buildHtml(template = "<html></html>", mappedVars = {}) {
  let vars = {};
  Object.assign(vars, defaultHtmlVals);
  Object.assign(vars, mappedVars);
